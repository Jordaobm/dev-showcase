...
export const MediaRecorderVideoAPISection = () => {
  ...
  const [isDisplayMediaSupported, setIsDisplayMediaSupported] =
    useState(true);

  useEffect(() => {
    setIsDisplayMediaSupported(!!navigator.mediaDevices?.getDisplayMedia);
  }, []);
  ...
  const shareScreen = async () => {
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        setError(t("mediaCapture.screenGenericError"));
      }
    }
  };
  ...
  const screenStatusLabel = (() => {
    if (shareScreenStatus) return t("mediaCapture.screenLivePreviewLabel");
    if (availableDownload) return t("mediaCapture.screenRecordedLabel");
    return t("mediaCapture.screenNoRecordingLabel");
  })();

  return (
    <div id="pwa-media-recorder-video-api" className="mt-12">
      <h3 className="text-2xl font-bold bg-gradient-to-r from-red-600 to-orange-600 bg-clip-text text-transparent">
        {t("mediaCapture.screenSectionTitle")}
      </h3>

      <div className="text-gray-600">
        <p>{t.rich("mediaCapture.screenDesc1", renderHtmlText)}</p>
        <br />
        <p>{t("mediaCapture.screenDesc2")}</p>
        <br />
        <p>{t("mediaCapture.screenCanDoIntro")}</p>
        <br />
        <ul className="space-y-1 list-disc list-inside">
          <li>{t("mediaCapture.screenCanDoItem1")}</li>
          <li>{t("mediaCapture.screenCanDoItem2")}</li>
          <li>{t("mediaCapture.screenCanDoItem3")}</li>
          <li>{t("mediaCapture.screenCanDoItem4")}</li>
          <li>{t("mediaCapture.screenCanDoItem5")}</li>
        </ul>
        <br />
        <p>{t("mediaCapture.screenUseCasesDesc")}</p>
        <br />
        <p>{t("mediaCapture.screenPwaNote")}</p>
        <br />
        <p>{t("mediaCapture.screenPermissionNote")}</p>
      </div>
      ...
      <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded text-sm text-amber-900">
        <div className="p-4 bg-amber-50 border border-amber-200 rounded text-sm text-amber-900">
          <div className="flex items-start gap-2">
            ...
            <p className="text-sm leading-relaxed">
              {t.rich("mediaCapture.screenWarningText", renderHtmlText)}
            </p>
          </div>
        </div>

        <p className="mt-2 font-bold">{t("mediaCapture.howToUseLabel")}</p>
        <p className="mt-2">{t("mediaCapture.screenHowToUseText")}</p>
        ...
        <div className="mt-4">
          <div className="flex flex-col md:flex-row mt-4 gap-4 items-start">
            <Button
              type="primary"
              onClick={shareScreen}
              disabled={!isDisplayMediaSupported}
            >
              {shareScreenStatus ? (
                <>
                  <ScreenShareOff className="w-5 h-5" />
                  {t("mediaCapture.screenStopButton")}
                </>
              ) : (
                <>
                  <ScreenShare className="w-5 h-5" />
                  {t("mediaCapture.screenShareButton")}
                </>
              )}
            </Button>
        ...
        {!isDisplayMediaSupported && (
          <p className="mt-4 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3">
            {t("mediaCapture.screenUnsupportedMessage")}
          </p>
        )}
        ...
        <div className="mt-4">
          ...
          >
            <div className="relative w-full aspect-video bg-black">
              <video // NOSONAR: self-view ao vivo via getDisplayMedia, sem fonte de legenda real
                ref={videoRef}
                autoPlay
                playsInline
              ...
              {shareScreenStatus && (
                <div className="absolute top-3 right-3 flex items-center gap-1.5 bg-black/60 backdrop-blur-sm rounded-full px-2.5 py-1">
                  ...
                  <span className="text-[10px] font-semibold tracking-widest text-white uppercase">
                    {t("mediaCapture.screenRecordingBadge")}
                  </span>
                </div>
              )}
            </div>
      ...
      <div className="mt-4 p-4 border rounded-lg">
        ...
        <ul className="space-y-2 text-sm text-gray-700">
          <li>{t.rich("mediaCapture.screenMethod1", renderHtmlText)}</li>
          <li>{t.rich("mediaCapture.screenMethod2", renderHtmlText)}</li>
          <li>{t.rich("mediaCapture.screenMethod3", renderHtmlText)}</li>
          <li>{t.rich("mediaCapture.screenMethod4", renderHtmlText)}</li>
          <li>{t.rich("mediaCapture.screenMethod5", renderHtmlText)}</li>
          <li>{t.rich("mediaCapture.screenMethod6", renderHtmlText)}</li>
          <li>{t.rich("mediaCapture.screenMethod7", renderHtmlText)}</li>
          <li>{t.rich("mediaCapture.screenMethod8", renderHtmlText)}</li>
          <li>{t.rich("mediaCapture.screenMethod9", renderHtmlText)}</li>
          <li>{t.rich("mediaCapture.screenMethod10", renderHtmlText)}</li>
        </ul>
      </div>
    </div>
  );
